...
export interface PromotionRow {
  id: string; code: string | null; name: string; kind: string | null;
  promo_group: PromoGroup; priority: number; is_auto: boolean; is_active: boolean;
  conditions: Record<string, unknown>; effect: Record<string, unknown>;
...
export function dbPromoToDomain(row: PromotionRow, now: number = Date.now()): Promo {
  ...
  return {
    ...
    unmet: unmetReason,
    when: (ctx) => {
      const vipWindow = ctx.member === "gold" && row.vip_starts_at
        && Number.isFinite(Date.parse(row.vip_starts_at)) && now >= Date.parse(row.vip_starts_at)
        && (!row.ends_at || now <= Date.parse(row.ends_at));
      if (!inWindow && !vipWindow) return false;
