...
description: 在專案中建立 AI 輔助開發的完整結構（AGENTS.md、docs/、rules/、hooks、agents）
user-invocable: true
---
...
2. **現有設定偵測**：
   - 檢查 `AGENTS.md`、`docs/`、`.Codex/rules/`、`.Codex/hooks/`、`.Codex/agents/`、`.Codex/settings.json` 是否已存在
...
```
AGENTS.md                      # 專案概述 + 常用指令 + 關鍵規則 + @docs 引用
docs/
...
建立或更新 `.Codex/settings.json`，設定 sandbox 模式和基礎權限。此階段向使用者展示預設設定，確認後寫入。
...
如果 `.Codex/settings.json` 已存在，**合併** permissions 和 sandbox 設定，不覆蓋已有的 allow/deny 規則和 allowedDomains。
...
| --- | ----------------- | ----------------------- | ------------- | ------------------------------------------------------------------------------ |
| 1   | 敏感檔案保護      | `PreToolUse`            | `Edit\|Write` | 建立 `.Codex/hooks/protect-files.sh`，阻止編輯 `.env`、`*.lock`、`.sqlite` 等 |
| 2   | 自動格式化        | `PostToolUse`           | `Edit\|Write` | 編輯後自動執行 prettier/eslint --fix（偵測專案使用的格式化工具）               |
| 3   | 自動產生 API 文件 | `PostToolUse`           | `Edit\|Write` | 建立 `.Codex/hooks/auto-api-docs.sh`，編輯路由檔後自動產生 API 文件           |
| 4   | Compact 提醒      | `SessionStart`          | `compact`     | context 壓縮後重新注入專案關鍵規則（內容從 AGENTS.md 摘錄）                    |
| 5   | 自動跑測試        | `PostToolUse`           | `Edit\|Write` | 編輯原始碼後自動執行對應測試                                                   |
| 6   | 通知提醒          | `Notification` + `Stop` | `""`          | 等待確認時 macOS 通知 + 任務完成時通知加音效（afplay Glass.aiff）              |

### hooks 建立規則

- 需要腳本的 hook（#1、#2、#3、#5）自動建立 `.Codex/hooks/*.sh` 並 chmod +x
- hook 設定寫入 `.Codex/settings.json` 的 `hooks` 區塊
- 如果 `settings.json` 已存在 hooks，**合併**而非覆蓋
- 通知 hook（#6）建議放在 `~/.Codex/settings.json`（全域），skill 會詢問使用者偏好
...
每個 agent 建立為 `.Codex/agents/{name}.md`：
