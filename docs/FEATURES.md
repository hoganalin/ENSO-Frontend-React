...
| Philosophy（禪）| ✅ | 「香を聞く / 時を聞く / 己を聞く」背景圖（incene-bg.png）+ 桌機視差滾動 |
| CTA（一支香，一段呼吸）| ✅ | 背景圖（incene-breath-bg.png）+ 桌機視差滾動 + Seal「香」印章 |
...
| 提交訂單 | ✅ | createOrderApi → 取得 orderId → 導向 `/payment/mock/:orderId`（demo 假金流）→ checkout-success |
...
| Theme Slice | ✅ | `themeSlice.ts` 管理主題狀態（如後續需切換亮/暗）|
